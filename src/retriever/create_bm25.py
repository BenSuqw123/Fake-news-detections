...
def build_bm25_standalone():
    ...
    print("✂️ Đang tách từ tiếng Việt (Underthesea) cho các chunks...")
    corpus = [item.get("text", "") for item in chunks] 
    
    # 🔥 Dùng underthesea với format="text" để tạo dấu gạch dưới (VD: thương_mại điện_tử)
    tokenized_corpus = [
        word_tokenize(doc, format="text").lower().split() 
        for doc in tqdm(corpus, desc="Tokenizing")
    ]

    print("⚙️ Đang huấn luyện mô hình BM25 Okapi...")
    ...
    print("💾 Đang lưu database xuống ổ cứng...")
    with open(bm25_save_path, 'wb') as f:
        pickle.dump(data_to_save, f, protocol=pickle.HIGHEST_PROTOCOL)

    print(f"✅ HOÀN THÀNH! Hệ thống BM25 đã sẵn sàng tại: {bm25_save_path}")
